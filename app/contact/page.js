            ...
            <div className="bg-muted/30 p-8 rounded-3xl border">
              ...
              >
                <input 
                  ...
                  className="flex-grow bg-background border rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
                />
            ...
            {ticketData ? (
              <div className="bg-background border rounded-3xl p-8 shadow-sm">
                <div className="flex items-center justify-between mb-8 border-b pb-6">
                  <div>
                    <span className="text-xs font-bold text-primary tracking-widest uppercase">{t('contact.ticket.status')}</span>
                    <h2 className="text-2xl font-bold mt-1">{ticketData.ticketId}</h2>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div className={cn(
                      ...
                      ticketData.ticketStatus === 'open' ? "bg-blue-100 text-blue-700" : 
                      ticketData.ticketStatus === 'closed' ? "bg-slate-100 text-slate-700" : "bg-orange-100 text-orange-700"
                    )}>
                  ...
                  <div className="border-t pt-6">
                    ...
                    <div className="bg-muted/30 p-4 rounded-2xl rounded-tr-none mb-4 ml-auto max-w-[85%]">
                      <p className="text-xs font-bold mb-1 uppercase tracking-tighter opacity-60">
                        {t('contact.ticket.you')} • {new Date(ticketData.createdAt).toLocaleDateString()}
                      </p>
                    ...
                    {allReplies.length > 0 && (
                      <div className="space-y-4">
                        {allReplies.map((reply, i) => (
                          <div key={reply.id || i} className={cn(
                            ...
                            <p className="text-xs font-bold mb-1 uppercase tracking-tighter opacity-60">
                              {reply.author === 'admin' ? t('contact.ticket.support') : t('contact.ticket.you')} • {new Date(reply.createdAt).toLocaleDateString()}
                              {reply.isOptimistic && <span className="ml-1">(sending...)</span>}
                            </p>
                    ...
                    {/* Reply Form */}
                    {ticketData.ticketStatus !== 'closed' && (
                      <form onSubmit={handleAddReply} className="mt-6 pt-4 border-t">
                        <p className="text-sm font-medium mb-3">{t('contact.ticket.conversation')}:</p>
