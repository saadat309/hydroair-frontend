...
export const useChatStore = create(
  persist(
    (set, get) => ({
      ...
      updateInitialMessage: (newInitialMessageText) => {
        set((state) => {
          ...
          const updatedSessions = sessions.map((session) => {
            if (session.id === currentSessionId && session.messages.length > 0) {
              // Only update if the message text is actually different
              if (session.messages[0].text === newInitialMessageText) {
                return session; // No change, return original session to prevent re-render
              }
              return {
                ...session,
                messages: [
                  { ...session.messages[0], text: newInitialMessageText },
          ...
          // Only update state if sessions array is actually different (i.e. something was updated)
